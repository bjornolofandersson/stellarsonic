...
export class MixPlaylist implements Playlist<MusicMixTrack> {
  ...
  private static instance: MixPlaylist;

  public static fromContent(content: MusicMix) {
    if (!this.instance) {
      this.instance = new MixPlaylist(Stellarsonic.audioPlayer(), content.audio, content.tracks);
    }
